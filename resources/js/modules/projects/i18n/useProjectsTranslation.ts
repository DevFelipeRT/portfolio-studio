...
export interface UseProjectsTranslationResult {
  locale: string;
...
export function useProjectsTranslation(
  ...
  if (!context) {
    throw new Error('useProjectsTranslation must be used within an I18nProvider.');
  }
  ...
  const translateWithNamespace: TranslationFunction = (
    ...
    const resolved = projectsTranslator.translate(
      locale as Locale,
      namespace,
      key,
      parameters,
