...
export interface UseContactChannelsTranslationResult {
  locale: string;
...
export function useContactChannelsTranslation(
  ...
  if (!context) {
    throw new Error(
      'useContactChannelsTranslation must be used within an I18nProvider.',
    );
  }
  ...
  const translateWithNamespace: TranslationFunction = (
    ...
    const resolved = contactChannelsTranslator.translate(
      locale as Locale,
      namespace,
      key,
      parameters,
