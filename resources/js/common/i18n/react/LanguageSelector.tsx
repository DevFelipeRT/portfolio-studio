...
export function LanguageSelector({
  ...
  const { translate: translateI18n, locale: contextLocale } = useTranslation(
    I18N_NAMESPACES.i18n,
  );
  const { translate: translateLanguageSelector } = useTranslation(
    I18N_NAMESPACES.languageSelector,
  );
  ...
  const selectorLabel = translateLanguageSelector('label', 'Language');
  const ariaLabel = translateLanguageSelector(
    'ariaLabel',
  ...
  const formatLabel = (code: string): string =>
    translateI18n(`locales.${code}.label`, code);

  const formatShortLabel = (code: string): string =>
    translateI18n(`locales.${code}.short`, code);
