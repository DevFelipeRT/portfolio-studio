...
- Project status rendering is now centralized in a shared module-level status helper/badge, so table rows, overlays, forms, translation modals, filters, and public project cards all consume the same canonical status values. Labels live exclusively in frontend i18n; backend payloads only transport the scalar value. The current editorial set is `planned`, `in_progress`, `delivered`, and `maintenance`, keeping publication visibility and live-demo presence modeled separately through `display` and `live_url` (`resources/js/modules/projects/core/status.ts`, `resources/js/modules/projects/ui/ProjectStatusBadge.tsx`, `resources/js/modules/projects/ui/table/ProjectsRow.tsx`, `resources/js/modules/projects/ui/ProjectOverlay.tsx`, `resources/js/modules/projects/ui/ProjectCard.tsx`, `resources/js/modules/projects/ui/form/project/ProjectForm.tsx`, `resources/js/modules/projects/ui/TranslationModal.tsx`).
