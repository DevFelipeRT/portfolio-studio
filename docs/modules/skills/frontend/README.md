...
- The skill details overlay now uses the shared `ItemDialog` composition model for badges, metadata, and body content, aligning it with the broader admin overlay system (`resources/js/modules/skills/ui/SkillOverlay.tsx`, `resources/js/common/table/item-dialog/ItemDialog.tsx`).
...
That category details overlay also now follows the shared `ItemDialog` composition model, with metadata rendered in the common overlay header rather than ad hoc dialog content (`resources/js/modules/skills/ui/SkillCategoryOverlay.tsx`, `resources/js/common/table/item-dialog/ItemDialog.tsx`).
