...
- That channel details overlay now follows the shared `ItemDialog` composition model while intentionally keeping the module-specific `Active` / `Inactive` vocabulary instead of publication-oriented visibility copy (`resources/js/modules/contact-channels/ui/ContactChannelOverlay.tsx`, `resources/js/common/table/item-dialog/ItemDialog.tsx`).
