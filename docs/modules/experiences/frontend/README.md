...
- That details overlay now uses the shared `ItemDialog` composition model, with summary, visibility, locale, and metadata structured through the common header/body slots (`resources/js/modules/experiences/ui/ExperienceOverlay.tsx`, `resources/js/common/table/item-dialog/ItemDialog.tsx`).
