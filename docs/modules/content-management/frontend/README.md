...
- The page listing now uses the shared `common/table` card-header slot + pagination foundation, with the filter form rendered inside the table card header and pagination state driven by the flat Laravel paginator contract (`resources/js/modules/content-management/features/page-management/page/listing/PageList.tsx`, `resources/js/common/table/*`).
- Single-column sorting is wired through the query string using `sort` and `direction`, with sortable headers for `name` (`internal_name`), `title`, `slug`, `locale`, `status`, and `updated_at`; `name asc` is the default active sort, and invalid sort inputs fall back to the same module default order (`internal_name asc`, then `id asc`) (`resources/js/app/pages/content-management/admin/page-index/page.tsx`, `app/Modules/ContentManagement/Infrastructure/Repositories/PageRepository.php`).
- Search is resolved server-side against `title`, `internal_name`, and `slug`, while status uses the same three-state lifecycle as the domain resolver: `draft` (`is_published = false` and `published_at = null`), `published` (`is_published = true`), and `archived` (`is_published = false` and `published_at != null`) (`app/Modules/ContentManagement/Domain/Services/PageStatusResolver.php`, `app/Modules/ContentManagement/Infrastructure/Repositories/PageRepository.php`).
- The listing preserves `per_page` in filter, pagination, and sort navigations, and the shared footer exposes the current per-page selection through the flat paginator payload (`resources/js/app/pages/content-management/admin/page-index/page.tsx`, `resources/js/modules/content-management/features/page-management/page/filtering/query.ts`, `resources/js/modules/content-management/features/page-management/page/listing/PageList.tsx`).
